...
static struct tx_frame_buffer *_alloc_tx_frame_buffer(void)
{
    ...
    return (struct tx_frame_buffer*) blk->blk;
}
...
os_int8 device_mac_send_msg(enum device_message_type type, os_uint8 msg[], os_uint8 len)
{
    ...
    construct_device_uplink_msg(type, msg, len,
                                ...
                                LPWAN_DEVICE_MAC_UPLINK_MTU);
    frame_buffer->len = len;
...
static signal_bv_t device_mac_engine_entry(os_pid_t pid, signal_bv_t signal)
{
    ...
    /**
     * \remark only one timeout_timer _should_ exist when the mac engine is running.
     */
    static struct timer *_timeout_timer = NULL;
    static struct timer *update_beacon_timer = NULL;

    static struct timer *joining_timer = NULL;
    static struct timer *wait_beacon_timeout_timer = NULL;
    ...
    (void) _s_ack; // we don't use it currently
    (void) _s_op2;
    ...
    case DE_MAC_STATES_JOINING:
        ...
        if (signal & SIGNAL_MAC_ENGINE_WAIT_BEACON) {
            ...
            if (_beacon_info.is_join_allowed || _is_lost_beacon) {
                ...
                // set up the periodically timer to update beacon info
    ...
    case DE_MAC_STATES_JOINED:
        ...
        case DE_JOINED_STATES_WAIT_BEACON:
            if (signal & SIGNAL_LPWAN_RADIO_RX_TIMEOUT) {
                    ...
                    && _s_info->beacon_class_seq_id == mac_info.expected_class_seq_id)
                {
                    ...
                    {
                        os_uint32 _delay = (_s_info->beacon_section_length_us *
        ...
        case DE_JOINED_STATES_TX_FRAME:
            ...
            if (signal & SIGNAL_MAC_ENGINE_SEND_FRAME) {
                os_uint32 _span = (_s_info->beacon_section_length_us *
                                  (64 - mac_info.allow_tx_info.allow_msg_emergent))
                                  / 1000;
                os_uint32 _delay = hdk_randr(5, _span-50);
                os_timer_reconfig(tx_timer, this->_pid,
                                              ...
                                              os_int8 packed_ack_delay_num)
{
    ...
    if (cur_seq_id < (os_int8) (BEACON_MAX_SEQ_NUM-1 - packed_ack_delay_num))
        return cur_seq_id + packed_ack_delay_num;
    else
        return packed_ack_delay_num - (BEACON_MAX_SEQ_NUM-1 - cur_seq_id);
}
