...
{
    sx1278_WakeUpInit();
    //Rf_SetPreambleLengthPara(Rf_GetConfigPreamble(rfTrue));
    /* set rf state standby */
    gs_tRfState = RF_STANDBY;
}   /* Rf_WakeUpInit() */
...
RF_RTE_RET_t Rf_Routine( void *a_pvdArg )
{
    ...
    switch( gs_tRfState )
    {
        ...
        case RF_RX:
            /* do rx process */
            tResult = sx1278_RxProcess();
            
            /* if receive ok */
            if(RF_P_OK == tResult)
            {
                /* update data to buffer */
//                Dtu_GetRfBuffer(sx1278_GetRxBufferAddress(), sx1278_GetRxPacketSize());
...
RF_RET_t Rf_Send( rf_char *a_pcData, rf_uint16 a_u16Len )
{
    if(a_pcData)
    {
        ...
        haddock_memcpy(sx1278_GetBufferAddr(), a_pcData, a_u16Len);
        sx1278_SetPacketSize(a_u16Len);
...
rf_uint16 Rf_Get( rf_char *a_pcData, rf_uint16 a_u16Len )
{
    ...
    if(a_pcData)
    {
        ...
        haddock_memcpy(a_pcData, sx1278_GetBufferAddr(), u16Ret);
