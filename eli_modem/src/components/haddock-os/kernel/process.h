...
struct process {
    ...
    /**
     * FIFO queue, with fixed length. Contain pointers to struct ipc_msg.
     * \sa HDK_CFG_PROC_MAX_MSG_QUEUE_SIZE
     */
    struct ringbuffer *msg_queue;
#endif
...
#define process_create_with_msg_queue(f, priority, msg_queue_size) \
    __haddock_process_create(& this, _proc_name, (f), (priority), (msg_queue_size))
#else
#define process_create(f, priority) \
    __haddock_process_create(& this, _proc_name, (f), (priority), 0)
#endif
